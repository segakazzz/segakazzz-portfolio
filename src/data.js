export const works = [
  {
    title: 'Scene-It',
    text: 'sakjsldkf lksdjlkfjsdlkgjksdgsdlg sdlgsdl',
    technologies: 'Javascript, External API, CSS, HTML',
    url: 'https://segakazzz.github.io/scene-it-starter/',
    githubUrl: 'https://segakazzz.github.io/scene-it-starter/',
    img: './img/scene-it.png'
  },
  {
    title: 'GoGITER Adventures',
    text: 'sakjsldkf lksdjlkfjsdlkgjksdgsdlg sdlgsdl',
    technologies: 'Javascript, CSS, HTML',
    url: 'https://segakazzz.github.io/week5-group-exercise/testimonies.html',
    githubUrl: 'https://github.com/segakazzz/week5-group-exercise/',
    img: './img/go-giter-adventures.png'
  },
  {
    title: 'Mondrian',
    text: 'sakjsldkf lksdjlkfjsdlkgjksdgsdlg sdlgsdl',
    technologies: 'CSS only artwork',
    url: 'https://segakazzz.github.io/flexbox-layout-exercises/mondrian.html',
    githubUrl: 'https://github.com/segakazzz/flexbox-layout-exercises/',
    img: './img/mondrian.png'
  },
  {
    title: 'Anpanman',
    text: 'sakjsldkf lksdjlkfjsdlkgjksdgsdlg sdlgsdl',
    technologies: 'CSS only artwork',
    url: 'https://segakazzz.github.io/flexbox-layout-exercises/anpanman.html',
    githubUrl: 'https://github.com/segakazzz/flexbox-layout-exercises/',
    img: './img/anpanman.png'
  },
  {
    title: 'Tumbler Clone',
    text: 'sakjsldkf lksdjlkfjsdlkgjksdgsdlg sdlgsdl',
    technologies: 'Node.js, Mustache, CSS, HTML',
    url: 'https://digitalcrafts-tumblr-clone.herokuapp.com',
    githubUrl: 'https://github.com/tjeve/phase-2-project-tumblr-clone',
    img: './img/tumbler-clone.png'
  },
  {
    title: 'This Website',
    text: 'sakjsldkf lksdjlkfjsdlkgjksdgsdlg sdlgsdl',
    technologies: 'Node.js, Webpack, Pug, SCSS',
    githubUrl: 'https://github.com/segakazzz/segakazzz-portfolio',
    img: './img/this-website.png'
  },
  {
    title: 'NOAC US Website',
    text: 'sakjsldkf lksdjlkfjsdlkgjksdgsdlg sdlgsdl',
    technologies: 'Python(Flask), Jinja, JQuery',
    url: 'https://www.noac.us/',
    img: './img/noac-us-website.png'
  },
  {
    title: 'Hungry Eyes',
    text: 'sakjsldkf lksdjlkfjsdlkgjksdgsdlg sdlgsdl',
    technologies: 'Javascript, External API, CSS, HTML',
    url: 'https://digitalcrafts-ph-1566664527167.firebaseapp.com/',
    githubUrl: 'https://github.com/tjeve/Phase-1-Project',
    img: './img/hungry-eyes.png'
  },
  {
    title: 'Othello Game',
    text: 'sakjsldkf lksdjlkfjsdlkgjksdgsdlg sdlgsdl',
    technologies: 'React, Redux, SCSS',
    url: 'https://othello-game-react.firebaseapp.com/',
    githubUrl: 'https://github.com/segakazzz/react-project',
    img: './img/othello.png'
  },
]
